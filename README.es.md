...
> **De un vistazo:** 10 agentes · 6 gates humanas · 11 principios · 8 reglas de ejecución · modelos en 3 tiers (opus/sonnet/haiku) · local-first.
...
Eso instala los 10 agentes, las skills, los hooks, el comando `/startsnt` y el scaffolder de proyectos. ¿Repo privado? corre `gh auth login` una vez primero.
...
Luego, en una sesión rooteada en la carpeta de tu proyecto, corre `/startsnt` para iniciar el intake guiado.
...
| [docs/SKILLS.md](docs/SKILLS.md) | Skills y herramientas: smoke, rúbrica de marca, intake gating, scaffolding, evals, observabilidad. |
| [docs/PIPELINE.md](docs/PIPELINE.md) | El pipeline gated, las 6 gates humanas, el grafo de tareas, la clasificación de tamaño, y qué es mecánico vs norma. |
| [docs/PRINCIPLES.md](docs/PRINCIPLES.md) | Principios **P-01…P-11** y reglas de ejecución **RC-01…RC-08** (la constitución del estudio). |

Las piezas ejecutables viven en la raíz: `agents/` (los 10 agentes), `skills/` (21 skills), `hooks/`, `commands/` (`/startsnt`), `scripts/` (el scaffolder) y `templates/`. Las copias de referencia en español de los agentes están en [`i18n/es/`](i18n/es/).
...
- 🌐 Los agentes/skills/comando activos están en **inglés**; las copias de referencia en español viven en `i18n/es/`.
